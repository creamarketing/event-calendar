...
$(function() {
		...
		$(this).each(function() {
			...
			$.extend(opts, {
				onComplete: function(event, queueID, fileObj, response, data) {
					...
					jQuery(document).trigger('Uploadify_ready');
					jQuery(document).trigger('Uploadify_complete');
				},
				onAllComplete : function(event) {
					$e = $(event.currentTarget);
					$e.data('active',false);
					$e.parents('form').find('input[type=submit]').attr('disabled',false).removeClass('disabled');
					$container = $e.parents('.UploadifyField:first');
				...
				onSelectOnce: function(event, queueID, fileObj) {
					...
					$e.data('active',true);
					if(!$e.metadata().upload_on_submit) {
						$e.parents('form').find('input[type=submit]').attr('disabled',true).addClass('disabled');
					}
					jQuery(document).trigger('Uploadify_busy');
				},
				onCancel: function(event, queueID, fileObj, data) {
					...
					jQuery(document).trigger('Uploadify_ready');
					jQuery(document).trigger('Uploadify_cancel');
				}
			});
		...
		$.post(
			...
			function() {
				$t.parents("li:first").fadeOut(function() {
					...
					jQuery(document).trigger('Uploadify_delete');
				});
			}
		);
		return false;
	...
	$('.folder_select').find('input[type=submit]').live("click", function() {
		$t = $(this);
	...
	$('.folder_select input[type=submit]').livequery(function() {
